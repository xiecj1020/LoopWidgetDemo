...
/**
 * 这个类是根据androidx.recyclerview:recyclerview:1.2.1版本的{@link PagerSnapHelper}代码实现.
 * 最好更新下这个类的实现.
 *
 * @author 谢朝军
 */
public class LoopPagerSnapHelper extends LoopSnapHelper {
    private static final int MAX_SCROLL_ON_FLING_DURATION = 100; // ms

    // Orientation helpers are lazily created per LayoutManager.
    @Nullable
    private LoopOrientationHelper mVerticalHelper;
    @Nullable
    private LoopOrientationHelper mHorizontalHelper;
    ...
    public int findTargetSnapPosition(RecyclerView.LayoutManager layoutManager, int velocityX,
        ...
        final LoopOrientationHelper orientationHelper = getOrientationHelper(layoutManager);
    ...
    protected LinearSmoothScroller createSnapScroller(RecyclerView.LayoutManager layoutManager) {
        ...
        return new LoopLinearSmoothScroller(mRecyclerView.getContext()) {
            @Override
    ...
    private int distanceToCenter(@NonNull RecyclerView.LayoutManager layoutManager,
                                 @NonNull View targetView, LoopOrientationHelper helper) {
    ...
    @Nullable
    private LoopOrientationHelper getOrientationHelper(RecyclerView.LayoutManager layoutManager) {
        if (layoutManager.canScrollVertically()) {
    ...
    @NonNull
    private LoopOrientationHelper getVerticalHelper(@NonNull RecyclerView.LayoutManager layoutManager) {
        if (mVerticalHelper == null || mVerticalHelper.mLayoutManager != layoutManager) {
            mVerticalHelper = LoopOrientationHelper.createVerticalHelper(layoutManager);
        }
        return mVerticalHelper;
    }

    @NonNull
    private LoopOrientationHelper getHorizontalHelper(
            @NonNull RecyclerView.LayoutManager layoutManager) {
        if (mHorizontalHelper == null || mHorizontalHelper.mLayoutManager != layoutManager) {
            mHorizontalHelper = LoopOrientationHelper.createHorizontalHelper(layoutManager);
        }
